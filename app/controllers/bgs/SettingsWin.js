...
/* 
 * uncomment to reset the Bgs server
 */
...
if(new Date(new Date().getTime()) > new Date(lastUpdate + 86400000)){
	...
	Ti.API.info(JSON.stringify(Ti.App.Properties.getList('avalaibleServersBgs')));
	Ti.API.info(JSON.stringify(Ti.App.Properties.getList('serversBgs')));
	Ti.App.Properties.setList('avalaibleServersBgs',[]);
	Ti.App.Properties.setList('serversBgs',[]);
};
/****/
...
function addBgsServerCompleted(){
	var dialog = Ti.UI.createAlertDialog({
		//message : 'New ' + win.service + ' server added',
		message : String.format(L('AddServer_OkDialog_message'), "BGS"/*win.service*/),
		ok : 'OK',
		//title : 'Server added'
...
// Add Bgs Server
var addBgsServer = function(){	

	//var avalaibleServers = Ti.App.Properties.getList('avalaibleServersBgs', []);
	//if(avalaibleServers.length){		
		var serversBgs = Ti.App.Properties.getList('serversBgs', []);
		if(serversBgs.length){
			xmlText = serversBgs[0].getCapabilities;
			if(debug_mode){
				Ti.API.info("Bgs Server already exists");
				Ti.API.info(xmlText);
			};
			return;
		};
	...
	xhr.onload = function() {
		...
		*/
		...
		//Append to serversBgs array the new server
		var serversBgs = [];
		...
		for (var i = 0; i < wcsCoverageSummary.length; i++) {
			...
			covArray.push({id: wcsCoverageSummary.item(i).getElementsByTagName("CoverageId").item(0).textContent, xml : ""});
		};
		if(debug_mode)
...
/*******---->*/
		 ...
		 */
		function getDescribeCoverage(serversBgs, serverIndex,coverageId) {
			//Output data
			var xmlText = null;
			try {
				...
				xhr.onload = function() {
					...
					//Ti.API.info("----> coverageIndex < serversBgs[serverIndex].describeCoverageArray.length-1 ---> " + coverageIndex + " < " + (serversBgs[serverIndex].describeCoverageArray.length-1));
					
					if(coverageIndex<serversBgs[serverIndex].describeCoverageArray.length-1){
						coverageIndex++;
						getDescribeCoverage(serversBgs, serverIndex, serversBgs[serverIndex].describeCoverageArray[coverageIndex].id);
					}else{
						Ti.App.Properties.setList('serversBgs', serversBgs);
						addBgsServerCompleted();
					};
				};
...
/*******<----*/
	...
	var strRequest = 'http://earthserver.bgs.ac.uk/petascope/wcs2?Service=WCS&Request=GetCapabilities';
...
var showAvailableCoverage = function(){
	if(OS_IOS){
		var availableCoveragesWin = Alloy.createController("bgs/AvailableCoveragesWin", {
			navWin: navWin
		}).getView();
		navWin.openWindow(availableCoveragesWin);
	}else{
		var availableCoveragesWin = Alloy.createController("/bgs/AvailableCoveragesWin").getView();
...
var showServerCapabilities = function(){
	if(OS_IOS){
		var serverCapabilitiesWin = Alloy.createController("bgs/ServerCapabilitiesWin", {
			navWin: navWin
		}).getView();
		navWin.openWindow(serverCapabilitiesWin);
	}else{
		var serverCapabilitiesWin = Alloy.createController("/bgs/ServerCapabilitiesWin").getView();
...
var showCoverageDescriptionList = function(){
	if(OS_IOS){
		var coverageDescriptionListWin = Alloy.createController("bgs/CoverageDescriptionListWin", {
			navWin: navWin,
			xmlText : xmlText
		}).getView();
...
var wv = Ti.UI.createWebView({url:"/BGS.html"});
infoWin.add(wv);
if(OS_ANDROID){
