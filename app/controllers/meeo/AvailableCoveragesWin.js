...
var checkAvailableCoverage = function(){	
	...
	for(var i in describeCoverageArray){		
		if(describeCoverageArray[i].spatialEnvelope){
			...
			if( (cornerRegion.no.lng >= describeCoverageArray[i].spatialEnvelope.lowerCorner[0]) &&
				(cornerRegion.ne.lng <= describeCoverageArray[i].spatialEnvelope.upperCorner[0]) &&
				(cornerRegion.se.lat >= describeCoverageArray[i].spatialEnvelope.lowerCorner[1]) &&
				(cornerRegion.no.lat <= describeCoverageArray[i].spatialEnvelope.upperCorner[1]))
				{
...
var openDrawableMap = function(){
	...
	if(OS_IOS){
		var drawableMapWin = Alloy.createController("meeo/DrawableMapWin", {
			navWin: navWin
		}).getView();
