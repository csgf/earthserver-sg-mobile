var bgsServer = Ti.App.Properties.getList('serversBgs', []);
...
var loadCapabilities = function(){
	...
	for(var i in bgsServer){
		if(bgsServer[i].url === "http://earthserver.bgs.ac.uk/petascope/wcs2"){
			coverages = bgsServer[i].describeCoverageArray;
			xmlData = Titanium.XML.parseString(bgsServer[i].getCapabilities);
		};
		break;
	};
...
var tvClick = function(e) {
	...
	if (e.index == 0) {
		if(OS_IOS){
			var wcsMetadataWin = Alloy.createController("bgs/WcsMetadataWin", {
				navWin: navWin,
				xmlData : xmlData
		...
		if(OS_IOS){
			var coverageMetadataWin = Alloy.createController("bgs/CoverageMetadataWin", {
				navWin: navWin,
				//xmlData : xmlData,
				rowID : e.index-1,
